...
However, R^2 is equal to 0,0867 which means that our model is explained by 8,7% of the variance, which is not very satisfying. This means that other variables have an impact on antidepressant consumption.
...
summary(mod2)
```
Given the results, this confirms that unemployment is not significant and that this variable could be removed. That's why we will now now focus on teleworking and analize the link between this variable and antidepressant consumption with graphs. 
