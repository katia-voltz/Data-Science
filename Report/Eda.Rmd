...
```
In this matrix, we see that none of our variables were strongly correlated with our dependent variable (y) mental health.
...
In the correlation matrix we could see that some of our explanatory variables were particularly correlated. We will therefore check that there is not too much multicollinearity which could bias our results.
...
stargazer(Reg2.2, type = 'text')
```

With the *stargazer* function we can see a significant increase for one unit of  1.70310 * ValueAntidepressantConsumption for "Teleworking". This variable is significant with 2 stars wich is satisfying. However, "Unemployment" is not significant as it represents an increase for one unit of 0.04958 * ValueAntidepressantConsumption. 
...
## Question 3 : How public spending on health impacts mental health in OECD countries
...
```
```{r}
stargazer(Regression.1.Time, type = 'text')
```
...
Graph.3
```{r}
Graph.region <- ggplot(tab_1, aes(x=Value_Spending.on.health, y=Mental_health_Value, colour=Region, group=Entity, labels=Time)+ ylim(0, 20)) + geom_point() 
...
Graph.4
```{r}
Graph.region2 <- ggplot(tab_1, aes( x = Mental_health_Value , y = Value_Spending.on.health, colour=Region, group=Entity, labels=Time) + ylim(0, 20)) +
  geom_point()
