...
```
By creating a graph of antidepressant consumption over time, we can see that, for our second selection of countries, antidepressant consumption is increasing. We will see if Covid Cases or Deaths are correlated to this increase.  
...
```{r, echo = FALSE, message = FALSE}
tab_R2.1 <- ddply(Antidepressant_consumption2, .(LOCATION),
                                  function (d) {
...
```
Here we have calculated the growth each year until 2017 of the depression rate by country. We can indeed observe a growth over the years, but this increase is smaller and smaller over time.  We can also see it on the following graph: 
