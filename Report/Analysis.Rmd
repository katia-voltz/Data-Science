...
reactable(comparaison, columns = list(
  Growth2020 = colDef(
...
We will try to demonstrate a link between the Covid crisis and the depression rate in the following countries. It is true that in this table we are comparing data from 2 different years : we only have the depression rate in 2021, and the Covid data we have is only in 2020.
...
# Graph4: Consumption according to teleworking 
...
Indeed, we can observe an increase in …

```{r, echo = FALSE, message = FALSE}
ggplot(data = tab_R2.2, mapping = aes(x = Teleworking, y = Antidepressant_Consumption)) +
