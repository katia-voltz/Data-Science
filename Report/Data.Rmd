...
For our second theme around Covid, we want to analyze if Covid had an impact on antidepressant consumption in selected countries around the world. We found data on antidepressant consumption on the website :  : <https://www.oecd.org>, in the Pharmaceutical Market Section. 
...
```
Here too we have only kept the country, the year and the value, and renamed the columns.
```{r}
...
We now download the data by sorting them according to the first selected country list. 
```{r}
